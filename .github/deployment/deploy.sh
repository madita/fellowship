...
run_hook () {
    ...
    tar -xf "$artifacts_path" ".github/deployment/hooks/$hook_file_name" -O | bash -se -- "${hook_parameters[@]}"
...
on_exit() {
    ...
    if [[ "$release_activated" == true ]] && [[ "$script_status_code" -ne 0 ]]; then
        echo "::warning::Warning: the new release has been activated!"
    fi
...
for release_directory_path in "$releases_directory/"*/ ; do
    if [[ -e "$release_directory_path" ]] && ! [[ $release_directory_path =~ /[0-9]+/$ ]] ; then
       echo "::error::The name of existing release directory \"$release_directory_path\" is not fully numeric, this should never happen."
...
if [[ -d "$lock_directory_path" ]]; then
    echo "::error::The directory \"$lock_directory_path\" exists, this means another deployment is currently running."
...
if [[ ! -s "$real_env_file_path" ]]; then
    ...
    echo "::error::Your \"$real_env_file_path\" file is empty. Run the deployment again after you've filled it in."
...
run_hook "after-activation.sh"

# Keep only the 5 newest release directories.
for old_release_directory in $(ls "$releases_directory" | sort --numeric-sort --reverse | tail -n+6) ; do
