...
current_release_directory_path=$1
previous_release_directory_path=$2

has_flushed_opcache=false

opcache_reset_script_file_name="$(tr -dc 0-9a-f </dev/urandom | head -c 8).php"
...
on_exit() {
    ...
    if [[ "$has_flushed_opcache" == false ]]; then
        echo "::error::Failed to flush OPCache. The APP_URL in your .env file is set to \"$app_url\", is this correct?"
    fi
...
app_url=$(php artisan tinker --execute "echo rtrim(config('app.url'), '/')")

echo "Pinging \"$app_url/$opcache_reset_script_file_name\" to flush OPCache."
