...
#
# For a full explanation on how we tackle file permissions, please visit: https://sjorso.com/laravel-file-permissions

new_release_directory=$1
...
if [[ -z "$http_user" ]]; then
    echo "::error::Could not detect which user serves web requests. You must manually set the \"\$http_user\" variable in the \"set-file-permissions\" hook."
...
if [[ -z "$(command -v "setfacl")" ]]; then
    echo "::error::We need \"setfacl\" to properly set file permissions, but it is not installed. You can install it using \"sudo apt install acl\"."
...
if ! id -nG "$deployment_user" | grep -q "\b$http_user_group\b"; then
    echo "::error::Can not set file permissions. The \"$deployment_user\" user should be in the \"$http_user_group\" group. You can do this using \"sudo usermod -aG $http_user_group $deployment_user\"."

    exit 1
fi

filesystems_config_permission_1=$(php artisan tinker --execute "echo decoct(config('filesystems.disks.local.permissions.dir.public'))")
filesystems_config_permission_2=$(php artisan tinker --execute "echo decoct(config('filesystems.disks.public.permissions.dir.public'))")

if [[ "$filesystems_config_permission_1" != "775" ]] || [[ "$filesystems_config_permission_2" != "775" ]]; then
    echo "::error::You must set the default directory permissions to 0775 in your \"filesystems.php\" config file. This must be done for both the \"local\" disk and the \"public\" disk. With the current setting you will get file permission errors. Make sure you set them to 0775, not 775."
